...
export const highestDailyDispatchTool = tool(async () => {
  const data = await calculations();
  const item = data.dailyDispatch[0];
  ...
  return output({
    ...
    calculation: "Sum actual dispatch-detail quantity by customer and business date, then rank descending.",
  ...
  description: "Return the single highest customer dispatch total on one date using actual dispatch-detail rows, not order quantities.",
  schema: z.object({}),
});

export const customerMonthlyActivityTool = tool(async ({ customer, start_month, end_month }) => {
  const data = await calculations();
  const key = customer.toLowerCase().replace(/[^a-z0-9]/g, "");
  const names = [...new Set(data.monthlyActivity.map((row) => row.customer))];
  const exact = names.find((name) => name.toLowerCase().replace(/[^a-z0-9]/g, "") === key);
...
export const dataQualityTool = tool(async () => {
  ...
  return output({
    ...
    sources: [source("Multiple workbooks", "Data Trust rules", "Latest PostgreSQL import")],
  });
}, {
