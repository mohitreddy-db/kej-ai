...
Report each plan's lot mix quantities, blended Fe (equal to the target by construction), blended landed cost per MT and remaining stock caveats.
If the blend query returns no rows, do NOT retry variations. A blend can only hit targets between the lowest and highest lot Fe in stock. Run exactly one diagnostic query (min fe, max fe, sum of qty for the category from the same latest-snapshot stock) and then answer: the exact target is not achievable, state the achievable Fe range, and offer the nearest alternative (for a target below every lot: the lowest-Fe lot over-delivers Fe; for a target above every lot: the highest-Fe lot falls short). Stop after that answer.
