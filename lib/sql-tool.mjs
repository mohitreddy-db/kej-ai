...
export async function runBusinessSql(sqlValue) {
  ...
  try {
    ...
    if (!result.rows.length) return { status: "incomplete", rows: [], row_count: 0, error: "No matching records were found.", executed_sql: sql };
    ...
    const [sourceResult, issueResult] = await Promise.all([
      ...
      client.query(`SELECT rule_code, count(*)::integer AS count
        FROM governance.data_issue WHERE source_row_id = ANY($1::bigint[]) AND resolved_at IS NULL GROUP BY rule_code ORDER BY count(*) DESC, rule_code`, [sourceIds]),
    ]);
